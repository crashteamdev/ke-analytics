...
class MarketDbApiV2Controller(
    ...
    override fun productOverallInfo(
        ...
        return checkRequestDaysPermission(
            ...
            val productAdditionalInfo =
                productServiceAnalytics.getProductAdditionalInfo(
                    productId.toString(),
                    skuId.toString(),
            ...
            return@flatMap ResponseEntity
                .ok(
                    ProductOverallInfo200Response().apply {
    ...
    override fun productSkuHistory(
        ...
        return checkRequestDaysPermission(
            ...
            val productAnalytics =
                productServiceAnalytics.getProductAnalytics(
                    productId,
                    skuId,
                    fromTime.toLocalDateTime(),
                    toTime.toLocalDateTime(),
        ...
        return checkRequestDaysPermission(
            ...
            if (access == false) {
                return@flatMap ResponseEntity
                    ...
                    .build<Flux<GetProductSales200ResponseInner>>()
                    .toMono()
            }
            ...
            val productSalesAnalytics =
                productServiceAnalytics.getProductSalesAnalytics(
                    productIds,
                    fromTime.toLocalDateTime(),
                    toTime.toLocalDateTime(),
                )
            val productSales =
                productSalesAnalytics.map {
    ...
    override fun getSellerShops(
        ...
        ResponseEntity
            ...
            .doOnError { log.error(it) { "Failed to get seller shops" } }

    override fun getReportByReportId(
        reportId: String,
        exchange: ServerWebExchange,
    ...
    override fun getReportBySeller(
        ...
        val apiKey =
            exchange.request.headers["X-API-KEY"]?.first()
                ?: return ResponseEntity.badRequest().build<GetReportBySeller200Response>().toMono()
        val idempotenceKey =
            exchange.request.headers["Idempotence-Key"]?.first()
                ?: return ResponseEntity.badRequest().build<GetReportBySeller200Response>().toMono()
        val user =
            userRepository.findByApiKey_HashKey(apiKey)
                ?: return ResponseEntity.badRequest().build<GetReportBySeller200Response>().toMono()
        ...
        if (checkReportAccess == UserRestrictionService.RestrictionResult.PROHIBIT) {
            return ResponseEntity
                ...
                .build<GetReportBySeller200Response>()
                .toMono()
        }
        ...
        return if (report != null) {
            ResponseEntity
                .ok()
                .body(
                    GetReportBySeller200Response().apply {
    ...
    override fun getReportByCategory(
        ...
        val checkReportAccess =
            userRestrictionService.checkCategoryReportAccess(
                user,
                userReportDailyReportCount,
            )
        if (checkReportAccess == UserRestrictionService.RestrictionResult.PROHIBIT) {
            return ResponseEntity
                .status(HttpStatus.TOO_MANY_REQUESTS)
    ...
    override fun getReportStateByJobId(
        ...
        if (report != null) {
            val responseBody =
                GetReportStateByJobId200Response().apply {
                    this.reportId = report.reportId
                    this.jobId = UUID.fromString(report.jobId)
        ...
        return checkRequestDaysPermission(X_API_KEY, fromTimeLocalDateTime, toTimeLocalDateTime).flatMap { access ->
            if (access == false) {
                ...
                val categoryOverallAnalytics =
                    productServiceAnalytics.getCategoryOverallAnalytics(
                        categoryId,
                        fromTimeLocalDateTime,
                        toTimeLocalDateTime,
                ...
                return@flatMap ResponseEntity
                    .ok(
                        CategoryOverallInfo200Response().apply {
                            this.averagePrice =
    ...
    override fun sellerOverallInfo(
        ...
        return checkRequestDaysPermission(X_API_KEY, fromTimeLocalDateTime, toTimeLocalDateTime).flatMap { access ->
            if (access == false) {
                ...
                return@flatMap ResponseEntity
                    .ok(
                        SellerOverallInfo200Response().apply {
                            this.averagePrice =
                                categoryOverallAnalytics.averagePrice.setScale(2, RoundingMode.HALF_UP).toDouble()
                            this.orderCount = categoryOverallAnalytics.orderCount
                            this.productCount = categoryOverallAnalytics.productCount
                            ...
                            this.salesDynamic =
                                categoryOverallAnalytics.salesDynamic.map { chSellerOrderDynamic ->
                                    DynamicSales().apply {
    ...
    override fun categoryProductPosition(
        ...
        val productPositions =
            productService.getProductPosition(
                categoryId,
                productId,
                skuId,
        ...
        return ResponseEntity
            .ok(
                CategoryProductPosition200Response().apply {
    ...
    override fun getTrendingCategoryProducts(
        ...
        categoryId: Long,
        exchange: ServerWebExchange,
    ): Mono<ResponseEntity<Flux<TrendingProduct>>> =
        runBlocking {
            try {
                val fromDate = LocalDate.now().minusDays(14)
                val toDate = LocalDate.now()
                val access =
                    checkRequestDaysPermission(
                        X_API_KEY,
                        fromDate.atStartOfDay(),
                        toDate.atStartOfDay(),
                    ).awaitSingle()
                ...
                if (!access) {
                    return@runBlocking ResponseEntity.status(HttpStatus.FORBIDDEN).build<Flux<TrendingProduct>>().toMono()
                }

                val trendingProducts = categoryAnalyticsService.getTrendingCategoryProducts(categoryId)

                if (trendingProducts.isEmpty()) {
                    return@runBlocking ResponseEntity.notFound().build<Flux<TrendingProduct>>().toMono()
                }

                val trendingProductResponses =
                    trendingProducts.map {
                        TrendingProduct().apply {
                            this.productId = it.productId.toLong()
                            this.name = it.name
                            this.photoKey = it.photoKey ?: ""
                        }
                ...
                ResponseEntity.ok(trendingProductResponses.toFlux()).toMono()
            } catch (e: Exception) {
                log.error(e) { "Failed to get trending category products" }
                ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build<Flux<TrendingProduct>>().toMono()
            }
        }.toMono()
